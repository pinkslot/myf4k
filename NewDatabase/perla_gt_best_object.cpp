...
namespace db
{
	...
	// Setup entity fields
	void PerlaGtBestObject::setupFields()
	{
		// Set table
		setTable("gt_best_object");
		// Add fields
		addField("object_id");
		...
		// Set key
		addKey("best_ground_truth_id", false);
	...
	template<>
	vector<Point> PerlaGtBestObject::getField<vector<Point> >(string field_name)
	{
	...
	template<>
	void PerlaGtBestObject::setField<vector<Point> >(string field_name, const vector<Point>& value)
	{
		// Check it's a polygon field
		if(field_name != "contour" && field_name != "bounding_box")
		{
