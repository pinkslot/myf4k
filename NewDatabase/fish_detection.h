...
//
#ifndef FISH_DETECTION_H
#define FISH_DETECTION_H
...
namespace db
{
	...
	class FishDetection : public DBEntity
	{
		private:
		...
		// Constructor
		inline FishDetection() : DBEntity() { setupFields(); }
		inline FishDetection(DBConnection* conn) : DBEntity(conn) { setupFields(); }
	...
	template<typename T>
	T FishDetection::getField(string field_name)
	{
	...
	/*template<typename T>
	void FishDetection::setField(string field_name, const T& value)
	{
	...
	/*template<>
	void FishDetection::setField(string field_name, const vector<Point>& value);*/
