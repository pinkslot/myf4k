...
namespace alg
{
	...
	Eigenbackground::Eigenbackground(void)
	{
		// Setup parameters
		parameters.add<int>("threshold", 225);
		parameters.add<float>("high_low_ratio", 2);
		parameters.add<int>("history", 100);
		parameters.add<int>("embedded_dim", 20);
		// Set init flag to true
		init = true;
	}

	Mat Eigenbackground::nextFrame(Context& context)
	{
		// Get frame
		...
		if(init)
		{
			...
			params.HistorySize() = parameters.get<int>("history");
			params.EmbeddedDim() = parameters.get<int>("embedded_dim");
