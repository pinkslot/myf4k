...
MultiLayerBGS_impl::MultiLayerBGS_impl() : firstTime(true), showOutput(true), 
...
MultiLayerBGS_impl::~MultiLayerBGS_impl()
{
  std::cout << "~MultiLayerBGS()" << std::endl;
}

void MultiLayerBGS_impl::setStatus(Status _status)
{
  status = _status;
}

void MultiLayerBGS_impl::finish(void)
{
  if(bg_model_preload.empty())
  {
    bg_model_preload = "MultiLayerBGSModel.yml";
    saveConfig();
  }
...
void MultiLayerBGS_impl::process(const cv::Mat &img_input, cv::Mat &img_output)
{
  if(img_input.empty())
    return;
...
void MultiLayerBGS_impl::saveConfig()
{
...
void MultiLayerBGS_impl::loadConfig()
{
