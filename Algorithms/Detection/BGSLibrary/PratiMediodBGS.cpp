...
namespace alg
{
	PratiMediod::PratiMediod(void)
	{
		// Setup parameters
		parameters.add<int>("threshold", 30);
		parameters.add<float>("high_low_ratio", 2);
		parameters.add<int>("sampling_rate", 5);
		parameters.add<int>("history", 16);
		parameters.add<float>("weight", 5);
		// Set init flag to true
		init = true;
	}

	Mat PratiMediod::nextFrame(Context& context)
	{
		// Get frame
		...
		if(init)
		{
			...
			params.Weight() = parameters.get<float>("weight");
