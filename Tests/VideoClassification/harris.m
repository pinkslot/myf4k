...
%
% Usage:  [cim, r, c] = harris(im, sigma, radius, display)
%
% Arguments:   
...
%                     be 1-3.
%            display   - optional flag (0 or 1) indicating whether you want
...
function [cim, r, c] = harris(im, sigma, radius, display)
...
%   cim = (Ix2.*Iy2 - Ixy.^2) - k*(Ix2 + Iy2).^2; 
	...
	cim = (cim==mx)&(cim>0);       % Find maxima.

    %cim
    % find indices and values of nonzero elements
	[r,c] = find(cim);                  % Find row,col coords.
	if nargin==4 & display      % overlay corners on original image
